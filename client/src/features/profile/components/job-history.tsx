...
const JobHistory = () => {
    ...
    const formatDate = (dateString: string | Date) => {
        ...
        const locale = t('jobHistory.dateLocale'); // 'en-US' or 'da-DK'
    ...
    const formatServiceType = (serviceType: 'basic' | 'deluxe') => {
        return serviceType === 'basic' ? t('jobHistory.serviceTypes.basic') : t('jobHistory.serviceTypes.deluxe');
    };
    ...
    const loadJobHistory = async () => {
        try {
            ...
            setError(error.message || t('jobHistory.errors.loadFailed'));
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <section className="bg-white p-6 rounded-lg space-y-4 shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_3px_3px_rgba(0,0,0,0.1)]">
            ...
            {!error && jobs.length === 0 && (
                <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">{t('jobHistory.empty.title')}</p>
                    <p className="text-gray-400 text-sm mt-1">{t('jobHistory.empty.subtitle')}</p>
                </div>
            )}
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <p className="font-semibold text-sm text-gray-900">
                                        {points} {t('purchasedPacks.points')}
                                    </p>
                ...
                <div className="border-t border-gray-200 pt-4 mt-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                        <div>
                            ...
                            <p className="text-sm text-gray-600">{t('jobHistory.stats.totalJobs')}</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-cyan">
                                {jobs.reduce((total, job) => total + job.price, 0)}
                            </p>
                            <p className="text-sm text-gray-600">{t('jobHistory.stats.pointsConsumed')}</p>
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-blue-600">
                                {jobs.filter(job => job.serviceType === 'deluxe').length}
                            </p>
                            <p className="text-sm text-gray-600">{t('jobHistory.stats.premiumJobs')}</p>
                        </div>
                    </div>
                </div>
            )}
